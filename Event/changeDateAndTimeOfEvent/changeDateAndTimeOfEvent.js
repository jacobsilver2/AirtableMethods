...
function selectEventToChange (records) {
  const dateList = records.map(record => moment(record.fields.Date).format("dddd, MMMM Do YYYY - hh:mm a"))
  inquirer.prompt([
    {
      type: 'list',
      name: 'eventSelect',
    ...
    const answerAsUtc = moment(answers.eventSelect, "dddd, MMMM Do YYYY - hh:mm a").utc().format()
    const selectedRecord = records.filter(record => moment(record.fields.Date).isSame(answerAsUtc));
...
function selectDateAndTime(id) {
  ...
  const answerAsDate = moment(`${answers.date}, ${answers.time}`, 'MMMD,YYYY, h:ma').format();  
